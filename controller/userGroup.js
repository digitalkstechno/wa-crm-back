...
exports.createGroup = async (req, res) => {
  try {
    ...
    const group = await UserGroup.create({ name, description, color });
...
exports.getAllGroups = async (req, res) => {
  try {
    ...
    const [groups, total] = await Promise.all([
      UserGroup.find(query).sort({ createdAt: -1 }).skip(skip).limit(limit),
      UserGroup.countDocuments(query),
    ]);

    const groupsWithCount = await Promise.all(
      groups.map(async (g) => {
        const count = await User.countDocuments({ group: g._id });
...
exports.deleteGroup = async (req, res) => {
  try {
    ...
    if (!group) throw new Error('Group not found');
    await User.updateMany({ group: req.params.id }, { group: null });
...
exports.fetchAllGroups = async (req, res) => {
  try {
    const groups = await UserGroup.find().populate({
      path: 'members',
