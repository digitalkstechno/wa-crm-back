...
PaymentReminderSchema.index({ status: 1, scheduledAt: 1 }); // Cron worker: find due paymentReminders
PaymentReminderSchema.index({ createdBy: 1, status: 1 });    // API: user's paymentReminders by status
PaymentReminderSchema.index({ createdBy: 1, scheduledAt: -1 }); // API: user's paymentReminders sorted
